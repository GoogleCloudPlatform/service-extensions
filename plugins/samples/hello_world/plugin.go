...
func (ctx *httpContext) OnHttpRequestHeaders(numHeaders int, endOfStream bool) types.Action {
	// Instead of sending response immediately, let's modify the response
	return types.ActionContinue
}

func (ctx *httpContext) OnHttpResponseHeaders(numHeaders int, endOfStream bool) types.Action {
	// Set response headers
	proxywasm.ReplaceHttpResponseHeader("Content-Type", "text/plain")
	proxywasm.ReplaceHttpResponseHeader(":status", "200")
	return types.ActionContinue
}

func (ctx *httpContext) OnHttpResponseBody(bodySize int, endOfStream bool) types.Action {
	// Replace the response body with "Hello World"
	proxywasm.ReplaceHttpResponseBody([]byte("Hello World"))
	return types.ActionContinue
}
