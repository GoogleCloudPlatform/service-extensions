...
process_results() {
    ...
    if [ -f "$result_file" ]; then
        ...
        if command -v jq &> /dev/null; then
            ...
            jq -r '.statusCodeDistribution | to_entries | .[] | "  \(.key): \(.value)"' "$result_file"
            
            # Show error distribution if any
            if jq -e '.errorDistribution' "$result_file" > /dev/null; then
                echo "Error distribution:"
                jq -r '.errorDistribution | to_entries | .[] | "  \(.key): \(.value)"' "$result_file"
            fi
        else
