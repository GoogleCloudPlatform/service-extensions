...
"""Pure unit tests for the Agent Breakglass Decider policy engine. No gRPC
server, no Firestore, no network.

Lives in the shared `extproc/tests/` tree (flat `<name>_test.py` naming),
not under `extproc/example/agent_breakglass/`, so `pytest extproc/tests/`
picks it up along with every other example's tests.
"""
import unittest

from extproc.example.agent_breakglass.decider import Decider, Decision
from extproc.example.agent_breakglass.findings import (
    DetectionSource, Finding)


def _finding(
    agent_id="agent-1", severity=80,
    source=DetectionSource.VERTEX_ANOMALY_DETECTION,
):
  return Finding(
      agent_id=agent_id, severity=severity, rationale="test", source=source,
...
class TestDecider(unittest.TestCase):
  ...
  def test_blocks_when_above_threshold(self):
    decider = Decider(
        dry_run=False, exempt_agents=(), min_severity_vertex_anomaly=70,
  ...
  def test_dry_run_short_circuits_before_actuation(self):
    decider = Decider(
        dry_run=True, exempt_agents=(), min_severity_vertex_anomaly=0,
  ...
  def test_exempt_agent_never_blocked(self):
    decider = Decider(
        dry_run=False, exempt_agents=("agent-1",),
        min_severity_vertex_anomaly=0, min_severity_scc=0,
        min_severity_wiz=0)
  ...
  def test_thresholds_are_per_source(self):
    decider = Decider(
        dry_run=False, exempt_agents=(), min_severity_vertex_anomaly=90,
        min_severity_scc=30, min_severity_wiz=60)
    r1 = decider.evaluate(
        _finding(severity=50, source=DetectionSource.VERTEX_ANOMALY_DETECTION))
    self.assertEqual(r1.decision, Decision.BELOW_THRESHOLD)
    r2 = decider.evaluate(
        _finding(severity=50, source=DetectionSource.SECURITY_COMMAND_CENTER))
