...
if [ $? -eq 0 ]; then
    ...
    if command -v jq &> /dev/null; then
        ...
        total_count=$(jq '.count' "$OUTPUT_FILE")
        ...
        request_header_updates=$(jq '[.details[] | select(.response.request_headers.response.header_mutation.set_headers != null)] | length' "$OUTPUT_FILE")
        response_header_updates=$(jq '[.details[] | select(.response.response_headers.response.header_mutation.set_headers != null)] | length' "$OUTPUT_FILE")
    else
