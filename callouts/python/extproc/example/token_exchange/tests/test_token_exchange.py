...
examples in this collection (e.g. litellm_gateway_test.py).
"""
import base64
import json
import time
...
from service_callout_example import (
...
class TestTokenCache(unittest.TestCase):
    ...
    def test_set_then_get(self):
        ...
        cache.set(key, CachedToken(mutations={"authorization": "Bearer x"}, expires_at=time.time() + 60))
...
class TestJwtClaimExtraction(unittest.TestCase):
    def test_extracts_known_claims(self):
        jwt = _fake_jwt({"sub": "user-1", "email": "user@example.com", "groups": ["a", "b"]})
