...
| **Missing Authorization header** | A request with no `Authorization` header is denied with `"No Authorization token found."`. |
| **Invalid JWT signature** | An `InvalidTokenError` is caught for tampered tokens and the request is denied with `"Authorization token is invalid."`. |
| **Expired token** | An `InvalidTokenError` is caught for expired tokens and the request is denied with `"Authorization token is invalid."`. |
| **Bare token (no Bearer prefix)** | The last whitespace-split token is used as-is and validated normally if the signature matches. |
| **PEM file missing at startup** | An `IOError` is raised at construction if `./extproc/ssl_creds/publickey.pem` is not found; the server fails to start. |
