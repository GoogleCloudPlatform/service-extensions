...
HOST="localhost"
PORT="8181"  # Using the insecure port from main.go
...
# Request data for testing the add_body example
# This simulates a response body modification
REQUEST_DATA='{
  "response_body": {
...
TOTAL_REQUESTS=${1:-1000}      # First parameter or default to 1000
CONCURRENT=${2:-20}            # Second parameter or default to 20
OUTPUT_FILE="$SCRIPT_DIR/results/$(date +%Y%m%d_%H%M%S).json"

# Function to process and display test results
process_results() {
    local result_file=$1
    ...
    if [ -f "$result_file" ]; then
        ...
        if command -v jq &> /dev/null; then
            ...
            if [ "$count" -ne 0 ] && [ -n "$ok_count" ] && [ "$ok_count" != "null" ]; then
...
# Function to recursively copy proto files
# This handles dependencies between proto files
copy_proto() {
    local proto_file=$1
    ...
    grep '^import ' "$src_path" | while read -r line; do
        ...
        if [ ! -f "$import_target" ] && [ -f "$SRC_PROTO_DIR/$import_file" ]; then
...
# Goes up 4 directory levels from load_test/ to reach callouts/go/
